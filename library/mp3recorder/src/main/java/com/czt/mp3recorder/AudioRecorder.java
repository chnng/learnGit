...
public class AudioRecorder extends Thread {
    ...
    private int bufsize = AudioRecord.ERROR_BAD_VALUE;
    ...
    public void stopRecord(int action) {
        mShouldRecord = false;
        mShouldRun = false;
    ...
    private void cancel() {
        stopRecord(DataEncodeThread.ACTION_STOP_ONLY);
    }
...
//                        Log.e("没有拿到权限：系统录音audioRecord.startRecording() 异常  :" );
                ...
                if (readSize > 0) {
                    ...
                    if (maxDuration > 0 && mDuration >= maxDuration) {
                        Mp3RecorderUtil.postTaskSafely(new Runnable() {
                            @Override
                            public void run() {
                                mMyMp3Recorder.stop(DataEncodeThread.ACTION_STOP_ONLY);
                                if (mDurationListener != null) {
    ...
    private void init() {
        ...
        try {
            ...
            mEncodeThread.setOnMp3BufferListener(mOnMp3BufferListener);
...
//                            EventBus.getDefault().post(new AudioNoPermissionEvent());
    ...
    private DataEncodeThread.OnMp3BufferListener mOnMp3BufferListener;

    public void setOnMp3BufferListener(DataEncodeThread.OnMp3BufferListener onMp3BufferListener) {
