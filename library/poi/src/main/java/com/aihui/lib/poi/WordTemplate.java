...
//            return;
//        }
//        @SuppressWarnings("unchecked") Map<String, Object> parametersMap = (Map<String, Object>)
//                dataMap.get("parametersMap");
//
...
//                        if (tableCells.size() != 2 || tableCells.get(0).getText().indexOf
//                                ("##{foreach") < 0 || tableCells.get(0).getText().trim().length()
//                                == 0) {
//                            System.out.println("文档中第" + (curT + 1) + "个表格模板错误,模板表格第一行需要设置2个单元格，"
//                                    + "第一个单元格存储表格类型(##{foreachTable}## 或者 ##{foreachTableRow}##)"
//                                    + "，第二个单元格定义数据源。");
//                            return;
//                        }
//
...
//                            return;
//                        }
//                        @SuppressWarnings("unchecked") List<Map<String, Object>> tableDataList =
//                                (List<Map<String, Object>>) dataMap.get(dataSource);
...
//     */
//    public void addParagraphInDocFooter(XWPFParagraph templateParagraph, List<Map<String,
//            String>> list, Map<String, Object> parametersMap, int flag) {
//
//        if (flag == 0) {
...
//        if (matcher.find()) {
//            // 查找到有标签才执行替换
//            int beginRunIndex = xWPFParagraph.searchText("{", new PositionInParagraph())
//                    .getBeginRun();// 标签开始run位置
...
//                    insertNewRun.getCTR().setRPr(beginRun.getCTR().getRPr());
//                    // 设置文本
//                    key.append(beginRunText.substring(beginRunText.indexOf("{") + 1, beginRunText
//                            .indexOf("}")));
//                    String textString = beginRunText.substring(0, beginIndex) + getValueBykey(key
//                            .toString(), parametersMap) + beginRunText.substring(endIndex + 1);
